...
// ─── GITHUB CONTENTS API HELPERS (module scope — shared by OG-page image ──
// pushes and avatar art generation) ───────────────────────────────────────
const GH_TOKEN = process.env.GITHUB_TOKEN || process.env.GITHUB_PAT;

function ghRequest(method, url, body, token) {
...
// Push (create or update) a file in the mealwheeliq GitHub Pages repo.
// Re-fetches the SHA immediately before every PUT — stale SHAs cause 409s.
async function ghPush(path, content, message, token = GH_TOKEN) {
  const url = `https://api.github.com/repos/ReviveIQ/mealwheeliq/contents/${path}`;
  let sha = null;
  try {
    const c = await ghRequest('GET', url, null, token);
...
// One-time-generated PNG assets, stored as static files at predictable URLs
// (mealwheeliq.com/avatar-art/{key}.png) — no DB lookup needed, frontend just
// builds the URL from the same `key` used in AVATAR_CATALOG. Base characters
// deliberately start plain (no chef coat, no hat) — buying hat/apron/tool
// items is what visibly dresses them up, which mirrors the rank ladder's own
// "toaster novice to chef" story instead of duplicating a chef hat that's
// already baked into the base art.
...
// Backgrounds are meant to fill the whole card (opaque), everything else
// needs a real alpha channel so it can be layered on top of other art.
...
// Bryan triggers this manually, once, after which the art is permanent static
// files served free forever — same "AI generates once, GitHub Pages serves
// forever" pattern already used for recipe photos.
//
// Query params:
//   ?only=key1,key2   regenerate just these keys (cheap way to fix one bad asset)
//   ?force=true       regenerate even if the file already exists (default: skip)
//
// Runs sequentially, not in parallel — keeps OpenAI rate limits and Railway
// memory usage predictable for a route that's expected to run rarely.
app.get('/admin/generate-avatar-art', adminAuth, async (req, res) => {
  ...
  for (const key of keysToRun) {
    ...
    try {
      if (!force) {
        const check = await ghRequest('GET', `https://api.github.com/repos/ReviveIQ/mealwheeliq/contents/${ghPath}`, null, GH_TOKEN);
        if (check.status === 200) {
      ...
      // PNG output (not JPEG) — has to preserve the alpha channel for the
      // transparent layers to actually composite instead of showing a solid box.
      ...
      await ghPush(ghPath, finalB64, `art: generate avatar asset ${key}`);
